...
| D6  | **(2026-08-19)** Operator decision: enable Google OAuth at launch once the API `GOOGLE_CLIENT_ID` is provisioned and the browser origin is authorized. The web discovers the API-owned ID at runtime; mock Google remains prohibited in production and email/password remains available as a fallback.                                                                                                                                                                                                                                                                                                                         |
...
1. **Deployment** (open item #3): web on Vercel, API on a container host,
   `NEXT_PUBLIC_API_URL`/`API_INTERNAL_URL` as
   Vercel **build** variables (A-083). Requires §8.7. The API hostname
   question is answered for now: the project owns no domain, so the API ships
   behind its Tailscale Funnel HTTPS host and `api.ateva.com` DNS is
   permanently off the table (AGENTS.md open item 3, 2026-08-23).
...
| 7   | **Infra**: ~~who controls DNS for `api.ateva.com`?~~ **answered 2026-08-23 — nobody here; the project owns no domain** and ships on the Vercel web origin + Tailscale Funnel API host. Host = OCI (`vnic1`); registry = GHCR.                                                                                                                                                                                                                                                                                                                                                                                           | W4.1–4.2               |
| 8   | **Google OAuth**: **answered 2026-08-19 — enable at launch**. Provision `GOOGLE_CLIENT_ID` on the API and authorize the production browser origin in Google Cloud. The web discovers the API-owned value through `/auth/config`; mock Google remains prohibited in production.                                                                                                                                                                                                                                                                                                                                          | W4.6 credentials       |
