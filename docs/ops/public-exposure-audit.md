...
> security job also passed a verified full-history gitleaks scan in run
...
the repository's verified full-history gitleaks action in CI run
`32287316349`; that scan reported no new baseline entries
...
| Vercel token/project credentials               | No token is tracked or embedded in the current GitHub remote. Browser-authenticated project access is verified; the Preview is ready. The `ateva.com` attachments were removed after the operator confirmed the domain belongs to another organization. | No credential value is reported here; historical token rotation remains operator-owned.                                                             | Revoke any historical token if the Vercel account reports one; keep production access and visibility review separate. Registrar publication for `ateva.com` is withdrawn — the project does not own the name. |
...
- [x] Attach attempt for `ateva.com`/`www.ateva.com` (2026-08-19) recorded,
      then reversed: verification failed because the domains are linked to
      another organization's account — they were never ours. Attachments
      removed from the project 2026-08-23.
- [x] ~~Publish the `_vercel` TXT records and A/CNAME records at the
      registrar~~ — withdrawn 2026-08-23: the project does not own
      `ateva.com`. Web ships on `ateva.vercel.app` until an owned domain
      exists.
- [ ] Verify GitHub, Vercel, Dodo, OAuth, database, Redis, and webhook secret
      stores contain no stale or duplicated credentials.
- [x] Run a verified full-history gitleaks scan in CI run
