...
| Vercel token/project credentials               | No token is tracked or embedded in the current GitHub remote. Browser-authenticated project access is verified; Preview deployment `76PS12GyxL2bZ69XNwccfMxHndiL` is ready. | No credential value is reported here; historical token rotation remains operator-owned.                                                             | Revoke any historical token if the Vercel account reports one; keep production access and visibility review separate.            |
