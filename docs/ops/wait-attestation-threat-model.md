...
| Nonce replay                    | Server-issued nonce digest plus atomic session consumption.                                    | Run concurrent provider-delivery tests against the real database.                           |
...
| Database race                   | Transactional CAS, unique constraints, advisory/row locking where applicable.                  | Mutation-test the consume path and run concurrent integration tests.                        |
