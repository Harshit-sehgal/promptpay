-- WL-071: deterministic, non-provider XTS payout simulations.
CREATE TABLE "sandbox_payout_simulations" (
  ...
  CONSTRAINT "sandbox_payout_simulations_user_id_fkey"
    FOREIGN KEY ("user_id") REFERENCES "users"("id") ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT "sandbox_payout_simulations_account_id_user_id_environment_id_fkey"
  ...
  CONSTRAINT "sandbox_payout_simulations_currency_check" CHECK ("currency" = 'XTS'),
  CONSTRAINT "sandbox_payout_simulations_amount_check" CHECK ("amount_minor" > 0)
);
CREATE UNIQUE INDEX "sandbox_payout_simulations_account_id_idempotency_key_key"
