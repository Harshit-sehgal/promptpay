 ...
 * operator `.env` files and compose files still carry `WAITLAYER_*`. This schema
 ...
 *
 * Instead, an unset `ATEVA_*` variable falls back to its `WAITLAYER_*` twin, so
 ...
 * `NEXT_PUBLIC_WAITLAYER_PAYOUT_*`. Delete this shim once no environment sets
 * the old prefix.
 */
