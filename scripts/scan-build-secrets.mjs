 ...
 *
 * Defaults to scanning `apps/web/.next` and `apps/api/dist`. The development
 * subtree at `.next/dev` is generated by `next dev` and is excluded; the
 * production `.next/server` tree remains fully scanned.
 */
...
// `next dev` keeps its development compiler output under `.next/dev`. It can
// contain source/test fixtures and is never shipped by `next build`, while
// production server output lives under `.next/server` and remains scanned.
function isNextDevOutput(path) {
  const parts = resolve(path).split(sep);
  return parts.some((part, index) => part === '.next' && parts[index + 1] === 'dev');
}
...
function* walk(dir) {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      if (isNextDevOutput(path)) continue;
      yield* walk(path);
...
function scanDirectory(dir) {
  const findings = [];
  if (isNextDevOutput(dir)) return findings;
...
function main() {
  ...
  const targets =
    explicit.length > 0
      ? explicit
      : [join(ROOT, 'apps', 'web', '.next'), join(ROOT, 'apps', 'api', 'dist')];
  ...
  for (const target of targets) {
    ...
    if (isNextDevOutput(target)) continue;
