  ...
  const tx = {
    ...
    sandboxOperation: {
      findUnique: async ({ where }) => {
        const key = where.accountId_idempotencyKey.idempotencyKey;
        const existing = state.operations.get(key);
        return existing ? { ...existing } : null;
      },
      create: async ({ data }) => {
        const created = {
          id: `operation-${data.idempotencyKey}`,
        ...
        state.operations.set(data.idempotencyKey, created);
        return { ...created };
      },
      update: async ({ where, data }) => {
        const key = String(where.id).replace(/^operation-/, '');
        const existing = state.operations.get(key) ?? {
          id: where.id,
        ...
        state.operations.set(key, updated);
        return { ...updated };
      },
    ...
    sandboxPayoutSimulation: {
      findUnique: async ({ where }) => {
        const existing = state.simulations.get(
          where.accountId_idempotencyKey.idempotencyKey,
        );
      ...
      create: async ({ data }) => {
        const created = {
          ...
          destinationAlias: data.destinationAlias,
