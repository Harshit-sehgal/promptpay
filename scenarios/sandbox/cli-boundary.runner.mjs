...
try {
  ...
  if (caseName === 'hooks-disabled') {
    ...
    if (capability.supported || capability.trustStatus !== 'unverified' || installed.status !== 'degraded' || installed.changed)
    ...
    await Promise.all([
      runSupervisedCommand(['/definitely/missing/waitlayer-ai-executable']).then(
        ...
        (error) => {
          ...
          // The supervisor requires credentials before spawning, so a missing
          // executable rejects before any child process (or synthetic wait
          // state) is created. Both the pre-spawn auth rejection and a raw
          // spawn ENOENT are valid fail-closed outcomes: no telemetry may be
          // recorded for a command that never started.
          if (!message.match(/ENOENT|spawn|Not logged in/))
