...
function makeAdTrait({ telemetryEnabled: hasTelemetry, adsEnabled, launchMode = 'sandbox' }) {
  const waitStart = {
  ...
  Object.assign(trait, {
    prisma: {
      ...
      waitAttestationSession: {
        findFirst: async () => ({ id: 'scenario-attestation-session' }),
      },
      adImpression: {
        findFirst: async () => null,
      },
    ...
    runtimeConfig: {
      getWaitLaunchMode: async () => launchMode,
      // These fixtures exercise the PRODUCTION consent gates (telemetry/ads
      // opt-outs), not sandbox placement serving, so the environment kind must
      // stay off the sandbox serving path.
      getEnvironmentKind: () => 'development',
...
async function adsDisabled() {
  const result = await makeAdTrait({
    telemetryEnabled: true,
    adsEnabled: false,
    launchMode: 'earnings_enabled',
  }).requestAd(userId, {
    deviceId,
  ...
  if (result.reason !== 'ads_disabled' || result.ad !== null)
