...
async function run() {
  ...
  // The extension has no client-side quiet-hours module; the quiet-hours
  // policy lives in the API and is import-free, so the fixture bundles it
  // directly to exercise the same midnight-range semantics.
  const quietHours = await bundle('quiet-hours', 'apps/api/src/extension/quiet-hours.ts');
  if (mode === 'terminal') {
