...
| --- | -------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Build/monorepo       | PASS    | `pnpm run build` (web `next build` + api `nest build`) passes; `pnpm run typecheck`/`lint` 14/14 & 9/9 — web `next build` forced to `NODE_ENV=production` to avoid an env-leak prerender crash (2026-07-11, resolved)                                         |
...
| 7   | Payouts              | Partial | Lifecycle, partial allocations, provider-failure release, and production stub guards tested; PayPal Payouts, Stripe Connect, and Wise call their real APIs when configured, Razorpay, Payoneer, and Dodo Payments remain dev/test stubs blocked in production |
...
- Multi-provider architecture: Manual, PayPal Email, PayPal Payouts, Wise, Stripe Connect, Razorpay, Payoneer, Dodo Payments.
