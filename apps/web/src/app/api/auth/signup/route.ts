...
export async function POST(req: NextRequest) {
  try {
    ...
    const signupRes = await fetchApiJson(`${apiBaseUrl()}/auth/signup`, {
      method: 'POST',
    ...
    if (!signupRes.ok) {
      return applyRateLimitIdentity(
        NextResponse.json(signupData ?? {}, { status: upstreamStatus(signupRes.status) }),
        identity,
        req.headers,
      );
    }
    ...
    const response = NextResponse.json(
      stripAuthTokens({ ...(signupData as Record<string, unknown>), user: fullUser }),
      { status: 201 },
    );
    return applyRateLimitIdentity(
