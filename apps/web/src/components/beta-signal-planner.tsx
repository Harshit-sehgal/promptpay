...
type ResultId = 'signals' | 'screen-time' | 'impressions' | 'clicks';
...
function ResultPair({
  ...
  return (
    <div className="landing-planner-result flex flex-col gap-1" data-planner-result={id}>
      <span className="landing-planner-result-label font-mono text-[11px] uppercase tracking-[0.1em] text-surface-500">
        {caption}
      </span>
      <span
        key={value}
        className="landing-planner-result-value font-serif text-[46px] leading-none text-surface-950"
        data-planner-result-value=""
      >
        {value}
      </span>
...
function PlannerCapacityDoodle() {
  return (
    <svg
      aria-hidden="true"
      className="landing-planner-feedback__doodle"
      viewBox="0 0 160 24"
      role="presentation"
    >
      <path
        className="landing-planner-feedback__doodle-line"
        d="M2 14C23 4 38 20 58 13s32-10 49-1 29 5 43-5"
        pathLength={1}
      />
      <path className="landing-planner-feedback__doodle-arrow" d="m142 7 9 0-5 7" />
      <circle className="landing-planner-feedback__doodle-marker" cx="2" cy="14" r="2.25" />
    </svg>
  );
}

function PlannerFlowDoodle() {
  return (
    <svg
      aria-hidden="true"
      className="landing-planner-flow__sketch"
      viewBox="0 0 240 36"
      role="presentation"
    >
      <path
        className="landing-planner-flow__line"
        d="M2 20C31 5 52 30 78 18s45-11 72 1 48 5 82-9"
        pathLength={1}
      />
      <path className="landing-planner-flow__arrow" d="m220 8 14 2-9 10" />
      <circle className="landing-planner-flow__marker" cx="2" cy="20" r="2.3" />
    </svg>
  );
}
...
export function BetaSignalPlanner() {
  ...
  const [plannerInView, setPlannerInView] = useState(false);
  ...
  const plannerDirty = mode === 'developer' ? devDirty : advDirty;
  ...
  const resetButton = (dirty: boolean, onReset: () => void) => (
    <button
      ...
      onClick={onReset}
      disabled={!dirty}
      data-planner-control="reset"
      data-planner-state={dirty ? 'changed' : 'default'}
      className={`landing-planner-control landing-planner-reset rounded-full px-3.5 py-1.5 text-[13px] ${
        dirty
  ...
  return (
    <section
      ...
      data-planner="beta-signal"
      data-planner-mode={mode}
      data-planner-state={plannerDirty ? 'changed' : 'default'}
      className={`landing-card landing-planner mx-auto mt-14 max-w-[1180px] rounded-3xl border border-surface-200/70 bg-white px-8 py-7${plannerInView ? ' landing-planner--in-view' : ''}`}
    >
      <div className="mb-1 flex flex-wrap items-start justify-between gap-5">
        ...
        <button
          ...
          data-planner-control="mode-toggle"
          data-planner-state={mode}
          className="landing-planner-control landing-planner-mode-toggle inline-flex min-h-11 items-center rounded-full border border-surface-950 bg-transparent px-5 text-sm text-surface-950 hover:bg-surface-100/60"
        >
          {mode === 'developer' ? 'For advertisers' : 'For developers'}
        </button>
      </div>

      <div className="landing-planner-flow" aria-hidden="true">
        <PlannerFlowDoodle />
      </div>

      <div key={mode} className="landing-planner-mode-panel" data-planner-mode-panel={mode}>
        {mode === 'developer' ? (
          <>
                ...
                {PRESETS.map((p) => {
                  ...
                  return (
                    <button
                      ...
                      data-planner-control="preset"
                      data-planner-state={active ? 'selected' : 'idle'}
                      data-planner-value={p.id}
                      className={`landing-planner-control landing-planner-choice flex min-h-11 flex-col gap-1.5 rounded-2xl p-4 text-left transition-colors ${
                        active
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-[0.9fr_1.1fr]">
              <div className="flex flex-col gap-[18px]">
                ...
                <div className="landing-planner-field" data-planner-control="eligible-waits">
                  <NumberField
                ...
                <div className="landing-planner-field" data-planner-control="units-per-hour">
                  <NumberField
                ...
                <button
                  ...
                  data-planner-control="quiet-hours"
                  data-planner-state={quiet ? 'on' : 'off'}
                  className={`landing-planner-control landing-planner-switch flex min-h-11 items-center justify-between gap-4 rounded-xl p-4 text-left transition-colors ${
                    quiet
              ...
              <div className="flex flex-col gap-4 md:border-l md:border-surface-200 md:pl-8">
                <div
                  aria-live="polite"
                  aria-atomic="true"
                  className="landing-planner-results grid grid-cols-2 gap-5"
                  data-planner-results-mode="developer"
                >
                  <ResultPair
                    id="signals"
                    caption="Estimated output"
                ...
                <p
                  aria-live="polite"
                  aria-atomic="true"
                  data-planner-feedback={capBinds ? 'capacity-limited' : 'within-capacity'}
                  className={`landing-planner-feedback landing-planner-message m-0 rounded-[10px] border p-3.5 text-sm leading-relaxed ${
                    capBinds
                ...
                >
                  ...
                  {capBinds ? <PlannerCapacityDoodle /> : null}
                </p>
          ...
          <>
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-[0.9fr_1.1fr]">
              <div className="flex flex-col gap-[18px]">
                ...
                <div className="landing-planner-field" data-planner-control="campaign-budget">
                  <NumberField
                ...
                <div className="landing-planner-field" data-planner-control="target-cpm">
                  <NumberField
                    label="Target CPM"
                    value={cpm}
                    min={CPM_MIN}
                ...
                <div className="landing-planner-field" data-planner-control="expected-ctr">
                  <NumberField
              ...
              <div className="flex flex-col gap-4 md:border-l md:border-surface-200 md:pl-8">
                <div
                  ...
                  aria-atomic="true"
                  className="landing-planner-results grid grid-cols-2 gap-5"
                  data-planner-results-mode="advertiser"
                >
                  <ResultPair
                    id="impressions"
                    caption="Modelled reach"
                    value={impressions.toLocaleString()}
                    unit="modelled impressions"
                  />
                  <ResultPair
                    id="clicks"
                    caption="Input-based clicks"
                ...
                <p
                  aria-live="polite"
                  aria-atomic="true"
                  data-planner-feedback={costPerClick === null ? 'no-clicks' : 'cost-per-click'}
                  className="landing-planner-feedback landing-planner-message m-0 rounded-[10px] border border-surface-200 bg-surface-50 p-3.5 text-sm leading-relaxed text-surface-700"
                >
                  {costPerClick === null
                    ? 'At this click-through rate the campaign models fewer than one click — raise the budget or the rate to see a cost per click.'
                    : `That works out to $${costPerClick.toFixed(2)} per assumed click. This is a planning model from your inputs, not observed campaign performance or a quote.`}
                </p>

                <p className="m-0 border-t border-surface-200 pt-3.5 text-[13.5px] leading-relaxed text-surface-600">
                  Click-through is your own assumption — Ateva does not publish a benchmark it has
                  not measured. Nothing here is a quote, and no campaign is billed during the beta.
                </p>
              </div>
            </div>
          </>
