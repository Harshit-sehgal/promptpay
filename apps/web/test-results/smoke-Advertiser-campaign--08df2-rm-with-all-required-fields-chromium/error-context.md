...
- Name: smoke.spec.ts >> Advertiser campaign creation (authenticated) >> renders the new campaign form with all required fields
- Location: e2e/smoke.spec.ts:173:7
...
Locator: getByLabel('Campaign name')
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByLabel('Campaign name')
...
```ts
  77  |     await page.goto('/');
  78  |     const footer = page.locator('footer').first();
  79  |     await expect(footer).toBeVisible();
  80  |     // The footer contains a "Cookie Settings" button that opens consent prefs
  81  |     await expect(page.getByText('Cookie Settings').first()).toBeVisible();
  82  |   });
  83  | });
  84  |
  85  | test.describe('Authentication pages', () => {
  86  |   test('login page renders with email and password fields', async ({ page }) => {
  87  |     await page.goto('/auth/login');
  88  |     await expect(page.locator('h1').first()).toHaveText(/Welcome back|Sign in/i);
  89  |     await expect(page.locator('input[type="email"]').first()).toBeVisible();
  90  |     await expect(page.locator('input[type="password"]').first()).toBeVisible();
  91  |   });
  92  |
