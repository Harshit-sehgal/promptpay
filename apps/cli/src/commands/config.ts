...
export async function runConfig() {
  ...
  // Round 36: wrapped the menu loop in `while (true)` instead of recursive
  // `await runConfig()` calls. Each menu iteration returned to the top of the
  // current frame via a recursive self-call, adding a stack frame each cycle.
  // After ~50-80 menu interactions (V8 async stack limit) the process crashed
  // with "Maximum call stack size exceeded". A `while` loop uses O(1) stack.
  while (true) {
    try {
      ...
      // Loop back to menu for further changes via the outer while(true).
      console.log();
      continue;
    } catch (err: unknown) {
